...
export const RightHandPanel: React.FC<RightHandPanelProps> = ({ isMobileView }) => {
    const { id } = useParams()
    const actions = useActions()
    const { activeChatId, chatsById } = useSelector(state => state.chats)
    const { messagesLoading, messagesError } = useSelector(state => state.messages)
    ...
    useEffect(() => {
        if (initialRender.current && typeof id === 'string') {
                    ...
                    {activeChatId ? (
                        <>
                            ...
                            <div
                                ...
                                className="flex-1 min-h-0 overflow-y-auto px-4 space-y-4 py-4"
                                key={activeChatId}
                            >
                                ...
                                >
                                    <div className="flex-1 relative">
                                        ...
                                        <button
                                            ...
                                            disabled={!message.trim() || isThinking || !activeChatId}
                                        >
                                            <svg
